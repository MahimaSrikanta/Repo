...
Tree.prototype.BFSelect = function(filter) {
  ...
  //Create the BFS array and then apply the filter to the BFS array
  var result = [];
  var queue = []; //To track all the nodes by level
  queue.push(this.value);
  while(queue.length !== 0) {
      var currentNode =  (queue.shift()); // remove from front
      result.push(currentNode);
      for(var i =0; i< currentNode.children.length; i++) {
          queue.push(currentNode.children[i]) // add to the end
      }
  }
  console.log(result);
};
